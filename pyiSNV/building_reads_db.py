...
class SampleMapBuilder:
    ...
    def build_reads_db(self, R1_file, R2_file, ref_kmer_builder):
        ...
        convolved, reads_kmer_mask, reads_abun, reads_string = reads2kmer(
            ref_kmer_dict,
            ref_kmer_f_dict,
            ref_kmer_r_dict,
            R1_file,
        ...
        if os.path.isfile(R2_file):
            ...
            convolved2, reads_kmer_mask2, reads_abun_2, reads_string_2 = reads2kmer(
                ref_kmer_dict,
                ref_kmer_f_dict,
                ref_kmer_r_dict,
                R2_file,
        ...
        unique_kmers = unique_kmers[mask]
        counts = counts[mask]
        hf_snp_list, hf_abun = get_snp_kmer(
            unique_kmers,
            counts,
            ref_kmer_f_dict,
            reads_abun,
        ...
        lf_snp_list, lf_abun = get_snp_kmer(
            lf_kmer,
            lf_counts,
            ref_kmer_f_dict,
            reads_abun + hf_abun,
        ...
        self.border_snv_list = border_process(
            ref_kmer_f_dict, seq, convolved, reads_abun, kmer_length
        )
...
def reads2kmer(
    ref_kmer_dict, ref_kmer_f_dict, ref_kmer_r_dict, R1_file, kmer_length, downsample
):
    t0 = time.time()
    ...
    for i in range(len(fore_after_ind) - 1):
        if True in r_mask[fore_after_ind[i] : fore_after_ind[i + 1]]:
            ...
            filtered_convolved[ind : ind + length] = convolved[
                fore_after_ind[i] : fore_after_ind[i + 1]
            ]
            ind += length
    ...
    for i in range(len(fore_after_ind) - 1):
        if True in r_mask[fore_after_ind[i] : fore_after_ind[i + 1]]:
            length = fore_after_ind[i + 1] - fore_after_ind[i]
            filtered_rev_convolved[ind : ind + length] = rev_convolved[
                fore_after_ind[i] : fore_after_ind[i + 1]
            ]
            ind += length
...
def get_masked_kmer(kmers, shift_size, window_size=1):
    ...
    kmers_high_bits = (kmers >> 2 * (shift_size + window_size)) << 2 * (
        ...
        if masked_ref_kmer_dict.get(masked_kmer):
            ...
            if hf_kmers_counts[i] > snv_limit * (reads_abun[loc] + hf_kmers_counts[i]):
                snp_list.append(
                    [
                        loc,
                        decode(loc2kmer_dict[loc], kmer_length),
                        decode(hf_kmers[i], kmer_length),
                        hf_kmers[i],
                        hf_kmers_counts[i],
                    ]
                )
...
def batch_unique(kmers, nb_batch):
    ...
    for i in range(nb_batch - 1):
        unique_kmers, counts = np.unique(
            kmers[i * batch_size : (i + 1) * batch_size], return_counts=True
        )
        kmer_count_list.append([unique_kmers, counts])
        # print(i, time.time()-t0)
    unique_kmers, counts = np.unique(
        kmers[(nb_batch - 1) * batch_size :], return_counts=True
    )
    kmer_count_list.append([unique_kmers, counts])
            ...
            for j in range(1, kmer_length):
                ...
                for item in short_dict.get(short_kmer):
                    if right_length <= item - loc < 2 * kmer_length:
                        ...
                        var_seq = "{}{}".format(
                            decode(key, kmer_length),
                            decode(kmer, kmer_length)[kmer_length - j :],
                        )
                        if (
                            var_seq[: kmer_length + right_length + 1]
                            not in ending_seq_set
                        ):
                            # temp_seq = [decode(elem) for elem in temp_array[:j+1]]
                            temp_count = [
                                ending_kmer_dict.get(elem)
                                for elem in temp_array[: j + 1]
                            ]
                            ending_seq_set.add(
                                var_seq[: kmer_length + right_length + 1]
                            )
                            ending_snv_list.append(
                                [
                                    loc,
                                    ref_seq,
                                    var_seq,
                                    temp_array[: j + 1],
                                    np.median(temp_count),
                                ]
                            )
            ...
            for j in range(kmer_length - 1):
                ...
                for item in short_dict.get(short_kmer):
                    ...
                    if right_length <= loc - item < 2 * kmer_length:
                        ...
                        var_seq = "{}{}".format(
                            decode(kmer, kmer_length)[: j + 1], decode(key, kmer_length)
                        )
                        if (
                            var_seq[-right_length - kmer_length :]
                            not in starting_seq_set
                        ):
                            temp_count = [
                                starting_kmer_dict.get(elem)
                                for elem in temp_array[kmer_length - 2 - j :]
                            ]
                            # temp_seq = [decode(elem) for elem in temp_array[kmer_length-2-j:]]
                            starting_seq_set.add(var_seq[-right_length - kmer_length :])
                            starting_snv_list.append(
                                [
                                    item + 1,
                                    ref_seq,
                                    var_seq,
                                    temp_array[kmer_length - 2 - j :],
                                    np.median(temp_count),
                                ]
                            )
...
def build_reads_db_bk(
    ...
    border_snv_list = border_process(
        ref_kmer_f_dict, seq, convolved, reads_abun, kmer_length
    )
