...
def dict2table(the_dict, key_dtype=np.int64, value_dtype=np.int64, length=0):
    if length==0:
...
def reads2kmer(ref_kmer_dict, ref_kmer_f_dict, ref_kmer_r_dict, R1_file, \
    ...
    return convolved, r_mask, reads_abun
...
def border_control(ref_kmer_f_dict, seq, convolved, reads_abun, kmer_length):
...
def build_reads_db(seq, ref_kmer_dict, ref_kmer_f_dict, ref_kmer_r_dict, R1_file, R2_file='', \
                   kmer_length=21, snv_limit=0.04,downsample=2):

    t0=time.time()
    
    convolved, reads_kmer_mask, reads_abun = reads2kmer(ref_kmer_dict, ref_kmer_f_dict, ref_kmer_r_dict,  
                       R1_file, kmer_length, downsample)
      
    #reads_ref_kmer_dict=dict(reads_ref_kmer_array)

    if os.path.isfile(R2_file):
        
        convolved2, reads_kmer_mask2, reads_abun_2 = reads2kmer(ref_kmer_dict, ref_kmer_f_dict, ref_kmer_r_dict, 
                           R2_file, kmer_length, downsample)
    ...
    border_snv_list = border_control(ref_kmer_f_dict, seq, convolved, reads_abun, kmer_length)
