...
class SNVArea:
    def __init__(self, index, loc, ref_seq, var_seq, var_kmers, kmer_dim, pre_dim, sur_dim):
        self.loc=int(loc)
    ...
    def _get_ref_seq(self):
        ...
        if min(self.ref_length, self.var_length)-\
            (self.pre_dim+self.sur_dim)>1:
            self.is_simple_area=False
        else:
            self.is_simple_area=True
    ...
    def split_var_seq(self, threshold):
        ...
        while True:
            ...
            if i>=len(ref_seq):
                break
            if min(len(ref_seq[i]),len(var_seq[i]))<=1 or len(ref_seq[i])==len(var_seq[i]):
                continue

            lcs_seq, nb_lcstr=longestCommonSubstr(ref_seq[i], var_seq[i])
            ref_lcs_loc=ref_seq[i].find(lcs_seq)
                        
            if len(lcs_seq)>=threshold \
    ...
    def explain_by_snv(self, snv_set, indel_only=True):
        ...
        while True:
            ...
            if len(ref_seq[i])==len(var_seq[i]) or min(len(ref_seq[i]), len(var_seq[i]))>5:
                continue
            
            coverage_range=[loc+rel_loc-len(ref_seq[i]), loc+rel_loc]

            for snv_case in snv_set:
                if loc<=snv_case.loc<=coverage_range[1] and\
                   snv_case.loc+len(snv_case.ref_base)-1<=coverage_range[1]:
                    ...
                    if snv_case.type=='deletion':
                        ...
                        if 2*len(snv_case.ref_base)<len(''.join(ref_seq[temp_pos:i+1])):
                            continue
                            
                        if pos_diff<=0:
                            ref_seq[temp_pos:i+1]=[snv_case.ref_base]+list(temp_seq[len(snv_case.ref_base):])
                            
                            if len(ref_seq[temp_pos:i+1])!=\
                                len([snv_case.ref_base]+list(temp_seq[len(snv_case.ref_base):])):
                                length_diff=len([snv_case.ref_base]+list(temp_seq[len(snv_case.ref_base):]))-\
                                    len(ref_seq[temp_pos:i+1])
                                ref_seq[i:i+length_diff+1]=[''.join(ref_seq[i:i+length_diff+1])]
                        else:
                            ref_seq[i:i+pos_diff]=list(ref_seq[i][:pos_diff])+[snv_case.ref_base]
                            var_seq[i:i+pos_diff+1]=list(''.join(var_seq[i:i+pos_diff+1]))
                        ...
                        if 2*len(snv_case.var_base)<len(''.join(var_seq[temp_pos:i+1])):
                            continue
                            
                        if pos_diff<=0:
                            var_seq[temp_pos:i+1]=[snv_case.var_base]+list(temp_seq[len(snv_case.var_base):])
                            
                            if len(var_seq[temp_pos:i+1])!=\
                                len([snv_case.var_base]+list(temp_seq[len(snv_case.var_base):])):
                                length_diff=len([snv_case.var_base]+list(temp_seq[len(snv_case.var_base):]))-\
                                    len(var_seq[temp_pos:i+1])
                                var_seq[i:i+length_diff+1]=[''.join(var_seq[i:i+length_diff+1])]
                        else:
                            var_seq[i:i+pos_diff]=list(var_seq[i][:pos_diff])+[snv_case.var_base]
                            ref_seq[i:i+pos_diff+1]=list(''.join(ref_seq[i:i+pos_diff+1]))
    ...
    def recognise_dense_isnv(self):
        ...
        if 1<nb_snv<3 and min(diff_seq)+nb_snv-1==max(diff_seq):
            temp_ref_seq_list=list(''.join(ref_seq[min(diff_seq):max(diff_seq)+1]))
            temp_var_seq_list=list(''.join(var_seq[min(diff_seq):max(diff_seq)+1]))
        ...
        while True:
            ...
            if 1<nb_snv<3 and var_length!=ref_length:
                ...
                temp_ref_seq=list(ref_seq[i])[:nb_snv-1]+[ref_seq[i][nb_snv-1:]]
                temp_var_seq=list(var_seq[i])[:nb_snv-1]+[var_seq[i][nb_snv-1:]]
    ...
    def recognise_adjacent_snp(self):
        ...
        while True:
            ...
            if min(var_length, ref_length)>1 and var_length==ref_length:
                ...
                diff_seq=[ref_base for ref_base, var_base in zip(temp_ref_seq, temp_var_seq) \
        ...
        while True:
            ...
            if len(ref_seq[i])>len(var_seq[i]):
                ...
                if k>0:
                    ...
                    ref_seq[i-k:i+1]=[temp_seq[:len(ref_seq[i])]]+list(temp_seq[len(ref_seq[i]):])
                    nb_changes+=1
                ...
                if k>0:
                    ...
                    var_seq[i-k:i+1]=[temp_seq[:len(var_seq[i])]]+list(temp_seq[len(var_seq[i]):])
                    nb_changes+=1
        ...
        while True:
            ...
            if var_seq[i]!=ref_seq[i] and min(var_length, ref_length)<=1 and i>=1:
                if ref_length>var_length:
                    snv_type=' deletion '
                    iSNV_key=str(loc+index) + snv_type + ref_seq[i] + ' ' +var_seq[i]
                ...
                for j in range(len(var_seq[i])):
                    if var_seq[i][j]!=ref_seq[i][j]:
                        snv_type=' snp '
                        iSNV_key=str(loc+index+j) + snv_type + ref_seq[i][j] + ' ' +var_seq[i][j]
                        
                        pos1=var_index+j
                        pos1=min(max(shift_size, pos1), self.var_length-shift_size-1)
                        kmer_seq1=self.var_seq[pos1-shift_size:pos1+shift_size+1]
                        kmer1=ReadKmer(kmer_seq1, loc+pos1)
                        
                        pos2=var_index+j
                        pos2=min(max(shift_size, pos2), self.var_length-shift_size-1)
                        kmer_seq2=self.var_seq[pos2-shift_size:pos2+shift_size+1]
                        kmer2=ReadKmer(kmer_seq2, loc+pos2)
...
class SNVCase:
    ...
    def get_count(self, count_dict):
        ...
        for temp_tuple in self.kmer_tuples:
            ...
            temp_count=[count_dict.get(kmer.get_kmer_code(), 0) for kmer in temp_tuple]
        ...
        for snv_case in snv_set:
            if abs(snv_case.loc-self.loc)<kmer_dim and snv_case.type==self.type:
                if snv_case.type=='insertion':
                    ...
                    if len(lcs)/max(len(self.var_base), len(snv_case.var_base))>0.67:
                        return True
                elif snv_case.type=='deletion':
                    lcs, _=longestCommonSubstr(self.ref_base, snv_case.ref_base)
                    if len(lcs)/max(len(self.ref_base), len(snv_case.ref_base))>0.67:
                        return True
    ...
    while len(snv_area_list)>0:
        ...
        if snv_area.is_simple_area: 
            ...
            for iSNV_key in iSNV_keys:
                ...
                if snv_area not in processed_set and new_case.find_lcs(iSNV_dict.values(), kmer_dim):
...
def output_isnv(seq, iSNV_area_list, snp_list, border_list, qc_dir, high_frequency_kmer_dict, \
                output_file_name, kmer_dim, reads_abun, snp_limit, downsample, \
    ...
    f.write('Position	Refence	Variation	Frequency	Depth\n')
