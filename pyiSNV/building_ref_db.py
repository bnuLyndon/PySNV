...
class GenomeMapBuilder:
    ...
    def build_ref_db(self, ref_file):
        ...
        if kmer_length % 2 == 0:
            print(
                "ERROR: setting kmer length as a even number is currently not supported"
            )
        ...
        unique_kmers, index, counts = np.unique(
        ...
        unique_kmers, index, counts = np.unique(
            rev_kmers, return_index=True, return_counts=True
        )
