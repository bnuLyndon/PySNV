...
class VariantLocator:
    def __init__(self, seq, ref_kmer_dict, ref_kmer_f_dict, \
                 kmer_length, snv_limit=0.04, snv_length_limt=1000): 
        self.seq = seq
        ...
        self.snv_length_limt = snv_length_limt
        
    def build_map(self, high_frequency_kmer_dict, connection_mat, 
                  unique_long_kmers, reads_abun):
        return output_iSNV_area(self.seq, high_frequency_kmer_dict, 
                                self.ref_kmer_dict, self.ref_kmer_f_dict, \
                                connection_mat, unique_long_kmers, \
                                reads_abun, self.kmer_length, self.snv_limit, 
                                self.snv_length_limt)


def output_iSNV_area(seq, high_frequency_kmer_dict, ref_kmer_dict, ref_kmer_f_dict, \
                    connection_mat, unique_long_kmers, long_kmer_counts, reads_abun, \
                    kmer_length, snv_limit=0.04, snv_length_limt=300):
    t0 = time.time()
        ...
        while len(stack):
            ...
            if end_idx:
                ...
                if len(var_seq) >= 2*kmer_length:
                    ...
                    #    continue
                    if long_kmer_dict.get(check_seq_head, 0) < long_kmer_threshold or \
                        long_kmer_dict.get(check_seq_mid, 0) < long_kmer_threshold or \
                        long_kmer_dict.get(check_seq_tail, 0) < long_kmer_threshold: 
